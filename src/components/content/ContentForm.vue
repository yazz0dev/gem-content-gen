// src/components/content/ContentForm.vue
<template>
        ...
        <VForm @submit="handleSubmit" :validation-schema="formSchema" class="mb-3">
            <div v-for="(field, index) in fields" :key="index" class="mb-3">
                ...
                <div v-if="field.type === 'text'" class="d-flex align-items-start">
                    <Field :type="field.inputType || 'text'" :id="field.key" :name="field.key" class="form-control flex-grow-1" />
                    <template v-if="field.enhanceable">
                        <div class="form-check ms-3">
                            <input type="checkbox" :id="`${field.key}-enhance`" v-model="formData[`${field.key}Enhance`]"
                ...
                <div v-else-if="field.type === 'textarea'" class="d-flex align-items-start">
                    <Field as="textarea" :id="field.key" :name="field.key" :rows="field.rows || 3"
                ...
                <div v-else-if="field.type === 'list'" class="input-group">
                    ...
                    <button type="button" @click="addListItem(field.key)" class="btn btn-outline-secondary"
                        :id="`${field.key}-add-btn`">Add</button>
                    <div class="mt-2">
                 ...
                 <!-- Date Input -->
                <div v-else-if="field.type === 'date'" class="d-flex align-items-start">
                    <Field type="date" :id="field.key" :name="field.key" class="form-control flex-grow-1" />
                    <template v-if="field.enhanceable">
                        <div class="form-check ms-3">
                            <input type="checkbox" :id="`${field.key}-enhance`" v-model="formData[`${field.key}Enhance`]"
                                class="form-check-input">
...
<script>
    ...
    export default {
        ...
        emits: ['generate-content', 'timer-update'],
        setup(props, { emit }) {
            ...
            watch(() => props.contentType, (newContentType) => {
                ...
                fields.value.forEach(field => {
                    ...
                    if (field.type === 'list') {
                        ...
                        fieldSchema = yup.array().of(yup.string());//Validate List
                    } else {
...
<style scoped>
    ...
    .input-enhance-group {
        display: flex;
        align-items: center;
        /* Vertically center the input and checkbox */
    }
    ...
    .input-enhance-group .form-control {
        ...
        margin-right: 0.5rem;
        /* Add some space between the input and checkbox */
    }
</style>